  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json(
        { error: error?.message || "Option not found" },
        { status: 404 }
      );
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (formData.has("option_name"))
      updateData.option_name = formData.get("option_name");
    if (formData.has("option_icon"))
      updateData.option_icon = formData.get("option_icon");
    if (formData.has("placeholder"))
      updateData.placeholder = formData.get("placeholder");
    if (formData.has("status")) updateData.status = formData.get("status");
    if (formData.has("input_type"))
      updateData.input_type = formData.get("input_type");
